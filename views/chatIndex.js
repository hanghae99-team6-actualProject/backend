...
socketMoim.on('connect', () => {
  const name = prompt('네임스페이스에서 쓸 대화명을 입력해주세요.', '');
...
const sendBtn = document.getElementById('chatMsgBtn'); // 전송버튼의 변수화
const chatInput = document.getElementById('chatInput'); // 채팅장의 변수화
...
socketMoim.on('updateMsg', (data) => {
  ...
  if (data.name === 'SERVER') {
    // html의 info를 가져와서 변수에 넣는다
    const infoContents = document.getElementById('info');
    ...
    // 서버(관리자)가 보낸 것이 아니라면 채팅창에 고정 노출
    const chatMsgData = drawChatMessage(data);
    chatWindow.appendChild(chatMsgData);
  }
});
