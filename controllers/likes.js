...
const {
  User, Like, Moim, Comment, MoimUser,
} = require('../models');
...
const createLike = async (req, res, next) => {
  try {
    ...
    await Like.create({ userId, moimId })
      .then(() => res.status(200).send({ result: true, msg: `${userId} 유저가 ${moimId} 게시글에 좋아요 생성` }));
  } catch (err) {
    logger.error(err);
    return next(err);
  }
...
const deleteLike = async (req, res, next) => {
  try {
    ...
    await Like.destroy({ where: { userId, moimId } })
      .then(() => res.status(200).send({ result: true, msg: `${userId} 유저가 ${moimId} 게시글에 좋아요 취소` }));
  } catch (err) {
    logger.error(err);
    return next(err);
  }
