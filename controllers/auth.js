...
const localLogin = async (req, res, next) => {
  try {
    ...
    return res.status(200).send({
      result: true, accessToken, refreshToken, msg: '로그인되었습니다.',
    });
  } catch (err) {
    logger.error(err);
    return next(err);
...
const signup = async (req, res, next) => {
  try {
    const {
      userEmail, userPw, userPwChk, nickName,
    ...
    await User.create({
      providerId, userEmail, userPw: encrypt(userPw), nickName, provider, exp, role,
    })
      .then(async (result) => {
