...
const userValidation = joi.object({
  userEmail: joi.string().min(3).max(30).pattern(new RegExp(emailFilter))
    .required()
...
const nickNameValidation = joi.object({
  nickName: joi.string().min(1).max(8)
    .error(new Error('닉네임은 1~8자로 입력하세요.')),
});
const userPwValidation = joi.object({
  userPw: joi.string().min(8).max(16).pattern(new RegExp(pwFilter))
    .error(new Error('비밀번호는 8~16자 영문 대 소문자, 숫자, 특수문자를 사용하세요.')),
});

module.exports = { userValidation, nickNameValidation, userPwValidation };
