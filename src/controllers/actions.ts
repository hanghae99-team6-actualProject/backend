...
//const { Request, Response, NextFunction } = require('express'); //무슨차이길래 오류가..
...
//루틴완료 경험치 계산 시 활용
let routineExp:number = 0;

//액션 루틴 완료 시 오늘 날짜 + 계정 기준으로 레코드 생성
const chkDayLog = async (userId:number): Promise<object> => {
  try {
    const fromToday = todaySet();
...
const upDayActionExp = async (userId:number): Promise<boolean> => {
  const fromToday = todaySet();
...
const upDayRoutineExp = async (userId:number, routineId:number): Promise<boolean> => {
  const fromToday = todaySet();
...
//당일 한계 경험치 체크
// const calcRoutineExp = async (userId:number, routineId:number): Promise<void> => {
...
const calcRoutineExp = async (userId:number, routineId:number): Promise<any> => {
  await Action.findAndCountAll({
    where: { userId, routineId },
  }).then((result:any):any => {
    routineExp = result.count * routineExpGrowth;
...
const upExpFinOneAction = async (userId:number): Promise<boolean> => {
  //당일 한계 경험치 체크함수
...
const upExpAllAction = async (userId:number, routineId:number): Promise<boolean> => {
...
const setActionFinDate = async (actionId:number, routineFinId:number, finDate:Date): Promise<void> => {
...
const setRoutineFinDate = async (routineId:number, finDate:Date): Promise<void> => {
  await RoutineFin.update({ date: finDate }, { where: { routineId } });
  logger.info('RoutineFin의 date 업데이트 완료');
};


const doneAction = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    // 프론트로부터 완료된 액션에 대한 정보 획득, 파라미터로 유저정보 획득

    //next 에러처리 변경되면 맞춰서 수정
